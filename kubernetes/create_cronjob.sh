...
cronjob_template_filepath="./cronjob.template.yaml"
cronjob_build_filepath="./cronjob.build.yaml"
...
if [[ $errorCode == 0 ]]; then
  ...
  sed -e "s;\${job\-name};$project_name;g" -e "s;\${full\-image\-name};$image_name;g" $cronjob_template_filepath > $cronjob_build_filepath

  # creating replication controller in the cloud
  kubectl create -f $cronjob_build_filepath --save-config
  ...
  if [[ $errorCode == 0 ]]; then
    ...
    # no errors returned
    # logging
    echo "New cron job has been created"

  else

    # error creating controller
    # apply modified replication controller
    kubectl apply -f $cronjob_build_filepath
